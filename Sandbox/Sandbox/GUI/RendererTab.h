	...
	public:
		...
		void Draw()
		{
			...
			if (ProjectSettings::instance()->getRendererType() == Hollow::RendererType::DirectX) {
				ImGui::Image(static_cast<D3D11RenderTarget*>(renderSystem->main)->GetShaderResourceView(), ImVec2(ImGui::GetWindowWidth(), ImGui::GetWindowHeight()));
			} else if (ProjectSettings::instance()->getRendererType() == Hollow::RendererType::OpenGL) {
				ImGui::Image((void*)static_cast<OGLRenderTarget*>(renderSystem->main)->texture, ImVec2(ImGui::GetWindowWidth(), ImGui::GetWindowHeight() - 16), ImVec2(0, 1), ImVec2(1, 0));
			}
			ImGui::End();
