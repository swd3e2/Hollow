#ifndef __POINTER_MATH_UTILS_H__
#define __POINTER_MATH_UTILS_H__
#include <cstddef>
#include <assert.h>
#include "Platform.h"
...
namespace Hollow { namespace Core { namespace Memory {
	...
	HOLLOW_API extern inline void* AlignForward(const void* address, unsigned char alignment)
	{
	...
	HOLLOW_API extern inline unsigned char GetAdjustment(const void* address, unsigned char alignment)
	{
		unsigned char adjustment = alignment - (reinterpret_cast<uintptr_t>(address) & static_cast<uintptr_t>(alignment - 1));
		return adjustment == alignment ? 0 : adjustment;
	}

	HOLLOW_API extern inline unsigned char GetAdjustmentWithHeader(const void* address, unsigned char alignment, unsigned char headerSize)
	{
